...
export default function GitLabFlowGuide() {
  return (
    <GuidePage><GuideHeader title="GitLab Flow 이해하기" audience="팀장 · 팀원" duration="약 10분">여러 기능을 어디에 모으고, 언제 배포할까요? GitLab Flow는 이슈와 작업 브랜치, 코드 리뷰, 배포 흐름을 연결하는 브랜치 전략입니다. 팀의 배포 방식에 맞춰 적용하는 방법을 알아봅시다.</GuideHeader><Callout id="flow-preparation" title="앞 가이드의 develop과 구분하기"><p>앞에서는 팀의 기준 브랜치 예시로 <code>develop</code>을 사용했습니다. 이 페이지는 GitLab Flow의 <code>main</code> 중심 구조를 설명합니다. 팀에서 사용 중인 브랜치는 이름만 바꾸지 말고, 역할과 배포 설정을 함께 합의하세요.</p></Callout><GuideBody reading><section>
          ...
          <FlowSteps label="GitLab Flow의 기본 작업 순서"><li><strong>Issue</strong><span>할 일과 완료 기준</span></li><li><strong>작업 브랜치</strong><span>main에서 분기</span></li><li><strong>PR / MR</strong><span>리뷰와 자동 검사</span></li><li><strong>main</strong><span>검증한 작업 통합</span></li></FlowSteps>
          ...
          <ReadingLinks>개념 출처: <a href="https://about.gitlab.com/topics/version-control/what-is-gitlab-flow/" target="_blank" rel="noopener noreferrer">GitLab Flow 공식 소개</a></ReadingLinks>
        </section><section>
          <StepTitle number={2}>작업과 배포 브랜치 구분하기</StepTitle>
          <ConceptGrid><div><h3>작업 브랜치 · 짧게 유지</h3><p><code>feature/login</code>, <code>fix/login-error</code>처럼 한 작업을 위한 공간입니다. main에서 만들고 PR이 합쳐진 뒤 정리합니다.</p></div><div><h3>main · 변경 통합</h3><p>리뷰와 검사를 마친 작업을 모읍니다. 새 작업은 최신 main에서 시작하고, main이 정상 동작하도록 관리합니다.</p></div></ConceptGrid>
          ...
          <CommandBlock>gh pr create --base main --head feature/login</CommandBlock>
        </section><section>
          <StepTitle number={4}>환경별로 배포하기</StepTitle>
          <p>다음은 <strong>환경 브랜치를 사용하는 팀의 예시</strong>입니다. 화살표는 변경을 다음 브랜치로 전달하는 방향입니다.</p>
          <FlowSteps label="환경 브랜치 예시: feature에서 main, staging, production 순서로 반영"><li><strong>feature/login →</strong><span>기능 개발</span></li><li><strong>main →</strong><span>리뷰한 기능 통합</span></li><li><strong>staging →</strong><span>배포 후보 검증</span></li><li><strong>production</strong><span>운영 반영</span></li></FlowSteps>
          <ul>
          ...
          <ReadingLinks>환경·버전별 확장: <a href="https://docs.gitlab.com/user/project/repository/branches/strategies/" target="_blank" rel="noopener noreferrer">GitLab 브랜치 전략 안내</a></ReadingLinks>
        </section><section>
          <StepTitle number={5}>버그 수정도 다음 배포에 포함하기</StepTitle>
          ...
          <GuideQuestion summary="여러 출시 버전을 동시에 지원한다면?"><p><code>stable/v1</code>, <code>stable/v2</code>처럼 버전별 브랜치를 유지하는 방법이 있습니다. 버그가 영향을 주는 버전을 확인하고 필요한 수정만 각 버전에 반영합니다. 이전 버전을 실제로 유지보수해야 할 때 도입하세요.</p></GuideQuestion>
        </section><section>
          <StepTitle number={6}>팀의 규칙으로 정리하기</StepTitle>
