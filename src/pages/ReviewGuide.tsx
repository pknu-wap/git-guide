...
export default function ReviewGuide() {
  return (
    <GuidePage><GuideHeader title="PR 리뷰 주고받기" duration="약 15분">리뷰는 변경의 목적과 동작을 팀원이 함께 확인하는 과정입니다. <br/> 리뷰 요청부터 피드백 반영과 승인까지 주고받아봅시다.</GuideHeader><GuideBody reading><section>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}preview_pull_request.png`} width={958} height={312} alt="GitHub Desktop에서 feat/kakao-login 브랜치의 Preview Pull Request 버튼이 표시된 화면. 새 탭에서 원본 이미지 보기." loading="lazy" caption="Preview Pull Request로 PR의 변경 내용 미리 확인하기" placement="inline" />
          <GuideScreenshot src={`${import.meta.env.BASE_URL}pull_request.png`} width={1778} height={1650} alt="GitHub의 카카오 로그인 구현 PR 화면: main과 feat/kakao-login 브랜치, 변경 내용, 관련 이슈, 검증 결과와 리뷰 참고 사항을 확인하는 예시. 새 탭에서 원본 이미지 보기." loading="lazy" caption="변경 내용과 검증 결과를 정리한 Pull Request 예시" placement="inline" />
        </section><section>
          <StepTitle number={2}>작성자: 리뷰어 지정하기</StepTitle>
          <p>PR 오른쪽의 <strong>Reviewers</strong>에서 확인을 부탁할 팀원을 선택하세요.</p>
          <GuideScreenshot src={`${import.meta.env.BASE_URL}reviewers.png`} width={630} height={160} alt="PR의 Reviewers 목록에 1lisalozf와 daimlee가 표시되고, 각 이름 오른쪽에 방패 아이콘과 노란 점이 있는 화면. 새 탭에서 원본 이미지 보기." loading="lazy" caption="Reviewers 예시" placement="inline" />
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}codeowners.png`} width={846} height={730} alt=".github/CODEOWNERS 파일에 유효한 설정 안내와 전체 파일 담당자를 지정하는 * @1lisalozf @daimlee @psvm203 규칙이 표시된 화면. 새 탭에서 원본 이미지 보기." loading="lazy" caption="CODEOWNERS에서 저장소 전체 파일의 리뷰 담당자 지정하기" placement="inline" />
          <p><code>*</code>은 전체 파일을, @psvm203은 담당자를 뜻합니다. 이렇게 설정하면 모든 파일 변경 시 해당 리뷰어가 지정되는데, 더 자세한 설명을 원하시면 하단의 자료를 참고하세요.</p>
          <ReadingLinks>참고 자료: <a href="https://docs.github.com/ko/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners" target="_blank" rel="noopener noreferrer">코드 소유자 정보</a></ReadingLinks>
        </section><section>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}review.png`} width={1780} height={722} alt="test.txt의 추가된 줄에 리뷰 댓글을 작성하고, 리뷰와 댓글에 Pending 상태가 표시된 화면. 새 탭에서 원본 이미지 보기." loading="lazy" caption="변경된 줄에 작성한 리뷰 댓글: 제출 전에는 Pending 상태로 표시됩니다." placement="inline" />
          ...
          <ul>
            ...
            <li><strong>Approve:</strong> 확인한 변경을 합쳐도 좋다는 승인을 남깁니다.</li>
            <li><strong>Request changes:</strong> 머지 전에 수정이 필요한 사항을 전달합니다.</li>
          </ul>
        </section><section>
          <StepTitle number={4}>작성자: 피드백 반영하기</StepTitle>
          <p>리뷰 댓글에 답한 후 필요한 내용을 <strong>기존 PR의 작업 브랜치</strong>에서 수정합니다.</p>
          <p>GitHub Desktop에서 현재 브랜치를 확인하고 수정 → 커밋 → 푸시하세요. 같은 브랜치로 푸시하면 기존 PR이 갱신되므로 새 PR을 만들 필요가 없습니다.</p>
          <p><strong>Reviewers</strong>에서 해당 리뷰어에게 재리뷰를 요청하세요. 해결된 대화는 <strong>Resolve conversation</strong>으로 정리합니다.</p>
          ...
          <p>리뷰어는 추가된 변경을 읽고 요청한 사항과 실행 결과를 확인합니다. 수정이 충분하다면 <strong>Approve</strong> 하세요.</p>
