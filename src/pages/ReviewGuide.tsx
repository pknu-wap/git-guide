  ...
  return (
    <div className="page development-guide">
      <section className="hero" aria-labelledby="page-title">
        ...
        <p className="intro">리뷰는 변경의 목적과 동작을 팀원이 함께 확인하는 과정입니다. README에 실행 방법을 추가하는 PR을 예로, 리뷰 요청부터 피드백 반영과 승인까지 주고받아봅시다.</p>
      </section>
      <aside className="preparation" aria-labelledby="review-preparation">
      ...
      <div className="instructions">
        <section>
          <h2 className="step-title"><span className="step-number">1</span>작성자 · 리뷰할 내용 준비하기</h2>
          <p>PR의 base가 팀의 기준 브랜치인지, compare가 내 작업 브랜치인지 확인하세요. 리뷰어가 이해하고 확인할 수 있도록 본문에 변경 이유와 검증 방법을 적습니다.</p>
          <pre className="command-block"><code>{`## 변경 목적
...
#1`}</code></pre>
        ...
        <section>
          <h2 className="step-title"><span className="step-number">2</span>작성자 · 리뷰어 지정하기</h2>
          ...
          <p className="example">요청 예시: “처음 설치하는 환경에서도 실행 방법을 따라 할 수 있는지 확인 부탁드립니다. 특히 환경 변수 준비 단계가 빠지지 않았는지 봐주세요.”</p>
        </section>
        <section>
          <h2 className="step-title"><span className="step-number">3</span>리뷰어 · 변경을 읽고 댓글 달기</h2>
        ...
        <section>
          <h2 className="step-title"><span className="step-number">4</span>리뷰어 · 의견을 정리해 제출하기</h2>
        ...
        <section>
          <h2 className="step-title"><span className="step-number">5</span>작성자 · 피드백 반영하고 재요청하기</h2>
          <p>리뷰 댓글에 답하고 필요한 내용을 <strong>기존 PR의 작업 브랜치</strong>에서 수정합니다. 동의하기 어렵거나 의도가 불분명하다면 이유를 설명하고 팀원과 방향을 맞추세요.</p>
          <ol className="git-flow" aria-label="리뷰 피드백 반영 순서">
          ...
          <p className="example">답변 예시: “환경 변수 준비 단계를 추가했고, 새 폴더에서 안내 순서대로 실행해 확인했습니다. 추가한 Commit을 다시 확인 부탁드립니다.”</p>
          ...
          <details className="git-question"><summary>Suggested change를 받았다면?</summary><p>제안된 수정이 맞는지 확인하고 GitHub의 <strong>Commit suggestion</strong>으로 반영할 수 있습니다. 이 경우 원격 작업 브랜치에 Commit이 생기므로, 로컬에서 계속 작업하기 전 작업을 정리하고 해당 브랜치에서 Fetch·Pull하세요.</p></details>
        </section>
        <section>
          <h2 className="step-title"><span className="step-number">6</span>리뷰어 · 수정 확인 후 승인하기</h2>
          ...
          <details className="git-question"><summary>연습은 어떻게 마무리하나요?</summary><p>README PR 하나에서 리뷰어가 수정 요청 → 작성자가 같은 브랜치에 수정 Push → 리뷰어가 재확인 후 승인하는 흐름을 완료하세요. 다음 PR에서는 역할을 바꾸면 양쪽 과정을 모두 익힐 수 있습니다.</p></details>
      ...
      <div className="guide-navigation">
        ...
        <a className="button" href="?guide=undo">다음 가이드: 실수한 변경 되돌리기 →</a>
      </div>
    </div>
  )
}
