  ...
  return (
    <div className="page development-guide">
      <section className="hero" aria-labelledby="page-title">
        ...
        <p className="intro">같은 파일의 같은 줄을 서로 다르게 수정하면 Git이 어느 내용을 남길지 결정하지 못할 수 있습니다. 두 브랜치에서 충돌을 직접 만들고, 변경 의도를 살려 합치는 과정을 GitHub Desktop으로 연습해봅시다.</p>
      </section>
      <aside className="preparation" aria-labelledby="conflict-preparation">
      ...
      <div className="instructions">
        <section>
          <h2 className="step-title"><span className="step-number">1</span>공통 시작점 만들기</h2>
          ...
          <pre className="command-block"><code>모임: 금요일</code></pre>
          <p>이 Commit을 공통 시작점으로 두 브랜치를 만들겠습니다. 브랜치를 바꾸기 전에는 매번 변경을 Commit해 <strong>Changes</strong>가 비어 있는지 확인하세요.</p>
        </section>
        <section>
          <h2 className="step-title"><span className="step-number">2</span>같은 줄을 다르게 수정하기</h2>
          <div className="concept-grid">
            <div><h3>A · 장소 추가</h3><p><code>practice/base</code>에서 <code>practice/place</code>를 만듭니다. README를 아래처럼 수정하고 Commit하세요.</p><pre className="command-block"><code>모임: 금요일, 동아리방</code></pre></div>
            <div><h3>B · 시간 추가</h3><p><code>practice/base</code>로 돌아온 뒤 <code>practice/time</code>을 만듭니다. README를 아래처럼 수정하고 Commit하세요.</p><pre className="command-block"><code>모임: 금요일, 오후 7시</code></pre></div>
          </div>
        ...
        <section>
          <h2 className="step-title"><span className="step-number">3</span>두 브랜치 합치기</h2>
          ...
          <p className="example">practice/place의 장소 정보 → practice/time의 시간 정보와 합치기</p>
        </section>
        <section>
          <h2 className="step-title"><span className="step-number">4</span>충돌 표시 읽고 내용 정리하기</h2>
          <p>충돌한 README를 에디터에서 열면 다음과 비슷한 표시가 나타납니다. 설정에 따라 브랜치 이름이나 공통 조상 내용이 추가로 표시될 수 있습니다.</p>
          <pre className="command-block"><code>{`<<<<<<< HEAD
모임: 금요일, 오후 7시
=======
모임: 금요일, 동아리방
>>>>>>> practice/place`}</code></pre>
          ...
          <pre className="command-block"><code>모임: 금요일, 오후 7시, 동아리방</code></pre>
        ...
        <section>
          <h2 className="step-title"><span className="step-number">5</span>결과 확인하고 병합 완료하기</h2>
          ...
          <details className="git-question"><summary>해결 방향을 모르겠다면?</summary><p>병합 완료 전에는 GitHub Desktop의 <strong>Abort merge</strong>로 이번 병합을 중단하고 팀원과 논의하세요. 충돌 해결 중 편집한 내용이 필요하면 별도로 복사해두세요. 터미널에서 시작한 일반 Merge는 <code>git merge --abort</code>로 중단할 수 있습니다.</p></details>
        </section>
        <section>
          <h2 className="step-title"><span className="step-number">6</span>실제 PR에 적용하기</h2>
          <p>PR에서 기준 브랜치와 충돌이 생겼다면, 먼저 진행 중인 작업을 Commit하거나 Stash해두고 아래 순서로 해결하세요. 기준 브랜치 이름은 팀에 맞게 바꿉니다.</p>
          <ol className="git-flow" aria-label="PR 충돌 해결 순서">
