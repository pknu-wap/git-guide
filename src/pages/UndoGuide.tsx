  ...
  return (
    <div className="page development-guide">
      <section className="hero" aria-labelledby="page-title">
        ...
        <p className="intro">파일 수정만 취소할지, Commit의 효과를 취소할지, Commit을 다시 구성할지에 따라 도구가 달라집니다. Discard Changes, Revert Changes in Commit, Reset의 차이와 사용 순서를 알아봅시다.</p>
      </section>
      <aside className="preparation" aria-labelledby="undo-preparation">
      ...
      <div className="instructions">
        <section>
          <h2 className="step-title"><span className="step-number">1</span>현재 상태에 맞는 방법 고르기</h2>
          ...
          <div className="table-scroll" tabIndex={0} role="region" aria-label="되돌리기 방법 비교">
        ...
        <section id="discard">
          <h2 className="step-title"><span className="step-number">2</span>Discard Changes · 미커밋 변경 버리기</h2>
          ...
          <details className="git-question"><summary>명령줄에서 같은 파일을 되돌리려면?</summary><p>아래 명령어는 HEAD에 존재하는 README의 스테이징과 작업 파일을 모두 마지막 Commit 상태로 맞춥니다. 이 파일의 미커밋 수정은 버려지므로 먼저 Diff를 확인하세요.</p><pre className="command-block"><code>{`git diff -- README.md
...
git restore --source=HEAD --staged --worktree -- README.md`}</code></pre><p><code>HEAD</code>는 현재 Commit입니다. 단순히 <code>git restore -- README.md</code>만 실행하면 작업 파일을 인덱스의 내용으로 돌리므로 이미 스테이징한 변경은 남습니다.</p></details>
          <p className="reading-links">공식 문서: <ExternalLink href="https://docs.github.com/en/desktop/making-changes-in-a-branch/committing-and-reviewing-changes-to-your-project-in-github-desktop">Discard Changes</ExternalLink> · <ExternalLink href="https://git-scm.com/docs/git-restore">git restore</ExternalLink></p>
        </section>
        <section id="revert">
          <h2 className="step-title"><span className="step-number">3</span>Revert Changes in Commit · 효과 취소하기</h2>
          ...
          <p className="example">기존 이력: A → B(잘못된 변경) → C<br />B를 Revert한 뒤: A → B → C → D(B의 반대 변경)</p>
          <p>되돌아가고 싶은 과거 시점을 선택하는 것이 아닙니다. B를 Revert해도 C의 변경까지 전부 취소되지는 않으며, C가 B에 의존한다면 추가 수정이나 충돌 해결이 필요할 수 있습니다.</p>
          <details className="git-question"><summary>터미널에서 마지막 Commit을 Revert하려면?</summary><p>작업 폴더가 깨끗한 상태에서, 취소할 대상이 마지막 일반 Commit인지 확인한 뒤 실행합니다.</p><pre className="command-block"><code>{`git status
git log -1 --oneline
git revert --no-edit HEAD`}</code></pre><p>다른 일반 Commit은 HEAD 대신 그 Commit의 해시를 지정합니다. 충돌이 나면 해결할 파일만 <code>git add</code>한 뒤 <code>git revert --continue</code>로 완료하세요. 진행 중인 Revert를 중단하려면 <code>git revert --abort</code>를 사용합니다. 병합 Commit은 부모 선택이 필요하므로 이 예시를 그대로 적용하지 마세요.</p></details>
          ...
          <p className="reading-links">공식 문서: <ExternalLink href="https://docs.github.com/en/desktop/managing-commits/reverting-a-commit-in-github-desktop">Revert Changes in Commit</ExternalLink> · <ExternalLink href="https://git-scm.com/docs/git-revert">git revert</ExternalLink></p>
        </section>
        <section id="reset">
          <h2 className="step-title"><span className="step-number">4</span>Undo / Reset to commit · Push 전 이력 정리하기</h2>
          ...
          <p className="example">A → B → C에서 A로 Reset<br />현재 브랜치 이력: A / B·C의 변경 내용: Changes에 남음</p>
          <p>선택한 A는 남고, 그 이후 Commit의 변경이 작업 폴더로 돌아옵니다. 필요한 내용을 다시 수정하고 선택해 Commit하세요. 버릴 내용만 별도로 Discard할 수도 있습니다.</p>
          <aside className="preparation note" aria-labelledby="desktop-reset-title"><div><h3 id="desktop-reset-title">Desktop의 Reset은 파일 수정을 보존합니다</h3><p>GitHub Desktop의 Reset to commit은 <code>git reset --hard</code>와 다릅니다. 마지막으로 Push한 Commit까지를 경계로 미Push Commit을 되돌릴 수 있습니다. 이미 공유한 Commit의 취소는 Revert로 진행하세요.</p></div></aside>
          <p className="reading-links">공식 문서: <ExternalLink href="https://docs.github.com/en/desktop/managing-commits/undoing-a-commit-in-github-desktop">Undo</ExternalLink> · <ExternalLink href="https://docs.github.com/en/desktop/managing-commits/resetting-to-a-commit-in-github-desktop">Reset to commit</ExternalLink></p>
        </section>
        <section>
          <h2 className="step-title"><span className="step-number">5</span>명령줄 Reset · soft, mixed, hard 구분하기</h2>
...
git branch backup/before-reset`}</code></pre>
          ...
          <div className="table-scroll" tabIndex={0} role="region" aria-label="Reset 옵션별 영향 비교">
            <table className="guide-table">
          ...
          <p className="reading-links">공식 문서: <ExternalLink href="https://git-scm.com/docs/git-reset">git reset 옵션</ExternalLink></p>
        </section>
        <section id="unstage">
          <h2 className="step-title"><span className="step-number">6</span>Unstage · git add만 취소하기</h2>
          <p>파일 수정은 필요하지만 이번 Commit에 넣고 싶지 않다면 스테이징만 해제합니다. 아래 명령어는 브랜치의 Commit 위치나 작업 파일 내용을 바꾸지 않습니다.</p>
          <pre className="command-block"><code>git restore --staged -- README.md</code></pre>
        ...
        <section>
          <h2 className="step-title"><span className="step-number">7</span>README로 연습하고 결과 확인하기</h2>
          <ul>
          ...
          <details className="git-question"><summary>Reset 대상을 잘못 골랐다면?</summary><p>추가 Reset을 멈추고 <code>git reflog</code>에서 Reset 전 Commit을 찾으세요. 해당 해시를 확인한 뒤 <code>git branch recover/work COMMIT_HASH</code>로 복구용 브랜치를 만들 수 있습니다. COMMIT_HASH는 찾은 해시로 바꿉니다. reflog는 로컬 기록이며, 미커밋 상태에서 버린 내용의 복구를 보장하지 않습니다.</p></details>
        </section>
      </div>
