...
export default function UndoGuide() {
  return (
    <GuidePage><GuideHeader title="실수한 변경 되돌리기" duration="약 20분">파일 수정만 취소할지, Commit의 효과를 취소할지, Commit을 다시 구성할지에 따라 도구가 달라집니다. Discard Changes, Revert Changes in Commit, Reset의 차이와 사용 순서를 알아봅시다.</GuideHeader><Callout id="undo-preparation" title="시작 전 준비사항"><p>별도 연습 레포지토리의 README로 진행하세요. 현재 브랜치와 Commit·Push 여부를 확인하고, 남길 수정은 파일 복사나 Stash로 보관합니다. Revert와 Reset 실습은 진행 중인 다른 변경이 없는 상태에서 시작하세요.</p></Callout><GuideBody reading><section>
        ...
        </section><section id="discard">
          <StepTitle number={2}>Discard Changes · 미커밋 변경 버리기</StepTitle>
          ...
          <GuideQuestion summary="명령줄에서 같은 파일을 되돌리려면?"><p>아래 명령어는 HEAD에 존재하는 README의 스테이징과 작업 파일을 모두 마지막 Commit 상태로 맞춥니다. 이 파일의 미커밋 수정은 버려지므로 먼저 Diff를 확인하세요.</p><CommandBlock>{`git diff -- README.md
git diff --cached -- README.md
git restore --source=HEAD --staged --worktree -- README.md`}</CommandBlock><p><code>HEAD</code>는 현재 Commit입니다. 단순히 <code>git restore -- README.md</code>만 실행하면 작업 파일을 인덱스의 내용으로 돌리므로 이미 스테이징한 변경은 남습니다.</p></GuideQuestion>
          <ReadingLinks>공식 문서: <ExternalLink href="https://docs.github.com/en/desktop/making-changes-in-a-branch/committing-and-reviewing-changes-to-your-project-in-github-desktop">Discard Changes</ExternalLink>· <ExternalLink href="https://git-scm.com/docs/git-restore">git restore</ExternalLink></ReadingLinks>
        </section><section id="revert">
          <StepTitle number={3}>Revert Changes in Commit · 효과 취소하기</StepTitle>
          ...
          <Example>기존 이력: A → B(잘못된 변경) → C<br />B를 Revert한 뒤: A → B → C → D(B의 반대 변경)</Example>
          <p>되돌아가고 싶은 과거 시점을 선택하는 것이 아닙니다. B를 Revert해도 C의 변경까지 전부 취소되지는 않으며, C가 B에 의존한다면 추가 수정이나 충돌 해결이 필요할 수 있습니다.</p>
          <GuideQuestion summary="터미널에서 마지막 Commit을 Revert하려면?"><p>작업 폴더가 깨끗한 상태에서, 취소할 대상이 마지막 일반 Commit인지 확인한 뒤 실행합니다.</p><CommandBlock>{`git status
git log -1 --oneline
git revert --no-edit HEAD`}</CommandBlock><p>다른 일반 Commit은 HEAD 대신 그 Commit의 해시를 지정합니다. 충돌이 나면 해결할 파일만 <code>git add</code>한 뒤 <code>git revert --continue</code>로 완료하세요. 진행 중인 Revert를 중단하려면 <code>git revert --abort</code>를 사용합니다. 병합 Commit은 부모 선택이 필요하므로 이 예시를 그대로 적용하지 마세요.</p></GuideQuestion>
          <p>이미 main이나 develop에 합친 변경을 되돌린다면 최신 기준 브랜치에서 새 수정 브랜치를 만들어 진행하세요. 원래 작업 브랜치만 수정해서는 기준 브랜치가 바뀌지 않습니다.</p>
          <ReadingLinks>공식 문서: <ExternalLink href="https://docs.github.com/en/desktop/managing-commits/reverting-a-commit-in-github-desktop">Revert Changes in Commit</ExternalLink>· <ExternalLink href="https://git-scm.com/docs/git-revert">git revert</ExternalLink></ReadingLinks>
        </section><section id="reset">
          <StepTitle number={4}>Undo / Reset to commit · Push 전 이력 정리하기</StepTitle>
          ...
          <Example>A → B → C에서 A로 Reset<br />현재 브랜치 이력: A / B·C의 변경 내용: Changes에 남음</Example>
          <p>선택한 A는 남고, 그 이후 Commit의 변경이 작업 폴더로 돌아옵니다. 필요한 내용을 다시 수정하고 선택해 Commit하세요. 버릴 내용만 별도로 Discard할 수도 있습니다.</p>
          <Callout id="desktop-reset-title" title="Desktop의 Reset은 파일 수정을 보존합니다" kind="note"><p>GitHub Desktop의 Reset to commit은 <code>git reset --hard</code>와 다릅니다. 마지막으로 Push한 Commit까지를 경계로 미Push Commit을 되돌릴 수 있습니다. 이미 공유한 Commit의 취소는 Revert로 진행하세요.</p></Callout>
          <ReadingLinks>공식 문서: <ExternalLink href="https://docs.github.com/en/desktop/managing-commits/undoing-a-commit-in-github-desktop">Undo</ExternalLink>· <ExternalLink href="https://docs.github.com/en/desktop/managing-commits/resetting-to-a-commit-in-github-desktop">Reset to commit</ExternalLink></ReadingLinks>
        </section><section>
          <StepTitle number={5}>명령줄 Reset · soft, mixed, hard 구분하기</StepTitle>
...
git log --oneline -5
git branch backup/before-reset`}</CommandBlock>
          <p>이미 같은 이름의 백업 브랜치가 있다면 다른 이름을 사용하세요. 아래 명령어는 <strong>비교용 대안이므로 하나만 선택</strong>합니다. 연속 실행하면 매번 다른 부모 Commit으로 이동합니다.</p>
          <GuideTable label="Reset 옵션별 영향 비교"><thead><tr><th scope="col">명령어</th><th scope="col">Commit 위치</th><th scope="col">변경 내용</th></tr></thead><tbody>
          ...
          <ReadingLinks>공식 문서: <ExternalLink href="https://git-scm.com/docs/git-reset">git reset 옵션</ExternalLink></ReadingLinks>
        </section><section id="unstage">
          <StepTitle number={6}>Unstage · git add만 취소하기</StepTitle>
          <p>파일 수정은 필요하지만 이번 Commit에 넣고 싶지 않다면 스테이징만 해제합니다. 아래 명령어는 브랜치의 Commit 위치나 작업 파일 내용을 바꾸지 않습니다.</p>
          <CommandBlock>git restore --staged -- README.md</CommandBlock>
          ...
          <StepTitle number={7}>README로 연습하고 결과 확인하기</StepTitle>
          <ul>
        ...
        </section></GuideBody><GuideNavigation><a href="?guide=review">← 이전 가이드: PR 리뷰 주고받기</a></GuideNavigation></GuidePage>
  )
}
