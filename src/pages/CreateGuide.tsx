...
export default function CreateGuide() {
  return (
    <GuidePage><GuideHeader title="(팀장) 팀의 레포지토리 만들기" audience="팀장" duration="약 5분">레포지토리(Repository)는 팀의 코드와 변경 이력이 담기는 공동의 저장소입니다. 팀장은 프로젝트 시작 시 레포지토리를 생성하고 팀원들에게 공유합니다.</GuideHeader><Callout id="preparation-title" title="시작 전 준비사항"><p>pknu-wap 조직에 초대되어야 합니다. 만약 초대되어 있지 않다면 임원진에게 문의해주세요.</p></Callout><LessonGrid><GuideBody><section id="lesson">
          ...
          </section></GuideBody><GuideScreenshot src={`${import.meta.env.BASE_URL}create_repository.jpg`} width={1548} height={1390} alt="GitHub 레포지토리 생성 화면: pknu-wap 조직, waps-server 이름, Public 공개 범위와 project-template 설정. 새 탭에서 원본 이미지 보기." reference={<ExternalLink href="https://github.com/pknu-wap/project-template">템플릿 원본 보기 <span aria-hidden="true">→</span></ExternalLink>} /></LessonGrid><section id="add-people" className="mt-16" aria-labelledby="add-people-title">
