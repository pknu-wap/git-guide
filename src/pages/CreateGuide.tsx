...
export default function CreateGuide() {
  return (
    <GuidePage><GuideHeader title="(팀장) 팀의 레포지토리 만들기" audience="팀장" duration="약 5분" icons>레포지토리(Repository)는 팀의 코드와 변경 이력이 담기는 공동의 저장소입니다. 팀장은 프로젝트 시작 시 레포지토리를 생성하고, 생성된 URL을 팀원들에게 공유합니다.</GuideHeader><Callout id="preparation-title" title="시작 전 준비사항" icon><p>pknu-wap 조직에 초대되어야 합니다. 만약 초대되어 있지 않다면 임원진에게 문의해주세요.</p></Callout><LessonGrid><GuideBody><section id="lesson">
            ...
            <Callout id="repository-structure-title" title="팀당 레포지토리가 하나일 필요는 없습니다." kind="note" icon><p>프로젝트 구성에 따라 여러 레포지토리를 만들 수 있습니다. 웹 프로젝트의 경우 프론트엔드와 백엔드 레포지토리를 분리하는 것을 권장합니다. 더 자세한 내용은 <strong>모노레포(Monorepo)</strong>와 <strong>폴리레포(Polyrepo)</strong>를 검색해보세요.</p></Callout>
