...
export default function CreateGuide() {
  return (
    <GuidePage><GuideHeader title="(팀장) 팀의 레포지토리 만들기">레포지토리(Repository)는 팀의 코드와 변경 이력이 담기는 공동의 저장소입니다. 팀장은 프로젝트 시작 시 레포지토리를 생성하고 팀원들에게 공유합니다.</GuideHeader><Callout id="preparation-title" title="시작 전 준비사항"><p>pknu-wap 조직에 초대되어야 합니다. 만약 초대되어 있지 않다면 임원진에게 문의해주세요.</p></Callout><LessonGrid><GuideBody><section id="lesson">
          ...
          </section></GuideBody><GuideScreenshot src={`${import.meta.env.BASE_URL}create_repository.jpg`} width={1548} height={1390} alt="GitHub 레포지토리 생성 화면: pknu-wap 조직, waps-server 이름, Public 공개 범위와 project-template 설정. 새 탭에서 원본 이미지 보기." reference={<ExternalLink href="https://github.com/pknu-wap/project-template">템플릿 원본 보기 <span aria-hidden="true">→</span></ExternalLink>} /></LessonGrid><section id="add-people" aria-labelledby="add-people-title">
        ...
        <GuideScreenshot src={`${import.meta.env.BASE_URL}add_people.png`} width={1892} height={736} alt="GitHub Manage access 화면: Add people 버튼과 팀원들의 Role: admin 권한 설정. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
        <Callout id="pending-invite-title" title="주의: Pending Invite가 표시되는 경우" kind="note"><p>별도의 절차 없이 팀원이 팀에 추가되어야 합니다. 아래처럼 <strong>Pending Invite</strong>가 표시된다면 해당 팀원이 <strong>pknu-wap</strong> 조직에 초대되지 않은 것이니 임원진에게 문의해주세요.</p></Callout>
        <GuideScreenshot src={`${import.meta.env.BASE_URL}pending_invite.png`} width={1864} height={166} alt="팀원 목록에 Pending Invite와 Awaiting response가 표시된 초대 대기 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
      </GuideBody></section><ButtonLink href="?guide=clone">다음 가이드: 팀의 레포지토리 복제하기 →</ButtonLink></GuidePage>
  )
}
