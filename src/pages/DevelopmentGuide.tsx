...
export default function DevelopmentGuide() {
  return (
    <GuidePage><GuideHeader title="Git으로 함께 개발하기" duration="약 15분">코드를 수정하고, 변경 이력을 남기고, 팀원의 작업과 합치는 과정을 알아봅시다.</GuideHeader><Callout id="development-preparation" title="시작 전 준비사항"><p>GitHub Desktop에서 팀 레포지토리를 열어주세요. 팀에서 작업을 합치는 Default 브랜치가 무엇인지 확인합니다.</p></Callout><GuideBody reading><section>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}issue.png`} width={1860} height={1268} alt="GitHub의 카카오 로그인 기능 구현 이슈 #1: 해결할 문제, 제안 내용, 완료 기준 체크리스트와 추가 정보가 작성된 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
        ...
        <p>아직 커밋하지 않은 작업이 있다면 먼저 커밋하거나 Stash로 잠시 보관한 뒤 브랜치를 바꾸거나 Pull하세요.</p>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}branch.png`} width={2144} height={1544} alt="GitHub Desktop의 Create a Branch 화면: main을 기준으로 feat/kakao-login 브랜치 이름을 입력하고 Create Branch 버튼으로 생성하는 예시. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}changes.png`} width={2144} height={1544} alt="GitHub Desktop의 Changes 화면: test.txt의 추가된 내용을 Diff로 확인하고 feat/kakao-login 브랜치에 Commit하는 예시. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
        </section><section>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}publish_branch.png`} width={900} height={102} alt="GitHub Desktop에서 Current Branch가 feat/kakao-login으로 선택되어 있고 Publish branch 버튼이 표시된 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
        </section><section>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}pull_request.png`} width={1778} height={1650} alt="GitHub의 카카오 로그인 구현 Pull Request 화면: feat/kakao-login에서 main으로 병합을 제안하고 변경 내용, 관련 이슈, 검증 결과와 리뷰 참고 사항을 작성한 예시. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
        </section><section>
