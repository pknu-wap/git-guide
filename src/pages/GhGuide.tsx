...
export default function GhGuide() {
  return (
    <GuidePage><GuideHeader title="GitHub CLI(gh) 사용하기" duration="약 10분">GitHub CLI는 터미널에서 GitHub의 기능을 사용하는 공식 도구입니다. <code>gh</code>명령어로 이슈를 확인하고 Pull Request를 만들어봅시다. 앞에서 배운 GitHub Desktop과 함께 사용할 수 있습니다.</GuideHeader><Callout id="why-github-cli" title="왜 GitHub CLI를 사용하나요?"><p>꼭 사용하지 않아도 됩니다. 하지만 이슈 생성, PR 작성, 검사 결과 확인처럼 자주 하는 작업을 빠르게 실행하고, 반복 작업은 스크립트로 자동화할 수 있습니다. 특히 AI를 사용해서 개발할 때 유용합니다.</p></Callout><GuideBody reading><section>
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}gh.png`} width={1140} height={640} alt="터미널에서 gh --version 명령어를 실행해 GitHub CLI 버전이 출력된 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}gh_login.png`} width={1138} height={802} alt="터미널에서 gh auth login으로 GitHub.com과 HTTPS, 웹 브라우저 로그인을 선택하고 Authentication complete와 로그인된 계정을 확인한 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
        ...
        <section className="mt-6">
          ...
          <GuideScreenshot src={`${import.meta.env.BASE_URL}zed.png`} width={3164} height={2068} alt="Zed에서 코드 편집기 아래에 내장 터미널을 연 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
          <h3 className="mt-8">VS Code</h3>
          <GuideScreenshot src={`${import.meta.env.BASE_URL}vscode.png`} width={3164} height={2068} alt="vscode에서 코드 편집기 아래의 Terminal 패널을 연 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
          <h3 className="mt-8">IntelliJ IDEA</h3>
          <GuideScreenshot src={`${import.meta.env.BASE_URL}intellij.png`} width={3164} height={2070} alt="IntelliJ IDEA에서 코드 편집기 아래의 Terminal 창을 연 화면. 새 탭에서 원본 이미지 보기." loading="lazy" placement="inline" />
