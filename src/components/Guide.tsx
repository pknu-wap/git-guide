...
export function GuideBody({ children, reading = false }: ContentProps & { reading?: boolean }) {
  return <div className={`guide-body text-(--muted) [&_li::marker]:text-(--forest) ${reading ? 'max-w-[850px]' : ''}`}>{children}</div>
}
...
export function CommandBlock({ children }: ContentProps) {
  return <pre className="rounded-[6px] border border-(--border) bg-(--pale-green) p-4 whitespace-pre-wrap wrap-anywhere"><code className="p-0">{children}</code></pre>
}
...
export function Example({ children }: ContentProps) {
  return <p className="guide-block rounded-[6px] bg-(--pale-green) p-4 text-(--forest)">{children}</p>
}
...
export function GuideScreenshot({ src, width, height, alt, loading, reference, placement = 'sticky' }: {
  ...
  return (
    <figure className={`guide-block top-24 min-w-0 [@media(width<=1100px)]:row-start-1 ${placement === 'sticky' ? 'sticky [@media(width<=1100px)]:static' : 'static'}`}>
...
export function GuideTable({ label, children }: ContentProps & { label: string }) {
  return (
    <div tabIndex={0} role="region" aria-label={label} className="guide-block overflow-x-auto focus-visible:outline-[3px] focus-visible:outline-solid focus-visible:outline-(--forest) focus-visible:outline-offset-[5px]">
...
export function ButtonLink({ href, external = false, children }: ContentProps & {
  href: string; external?: boolean
}) {
  const Link = external ? ExternalLink : 'a'
  return <Link href={href} className="inline-block shrink-0 rounded-[6px] bg-(--forest) px-6 py-3 text-center font-bold text-white no-underline hover:bg-[#28543d] hover:text-white max-[768px]:w-full">{children}</Link>
}

export function GuideNavigation({ children }: ContentProps) {
  return <div className="flex flex-wrap items-center justify-between gap-6">{children}</div>
}
